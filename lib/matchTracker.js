...
const getValveMatchDetails = async (matchId) => {
    ...
    try {
        const url = `http://api.steampowered.com/IDOTA2Match_570/GetMatchDetails/v1/?key=${process.env.STEAM_API_KEY}&match_id=${matchId}`;
        const body = await got(url).json();
        return body && body.result && !body.result.error ? body : null;
    }
    catch (e) {
        logger.error(e);
        return null;
